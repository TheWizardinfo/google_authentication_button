...
def login_is_required(function):
    ...
    decorator.__name__ = function.__name__  # Set the name of the inner function to avoid conflicts
    return decorator
...
def callback():
    ...
    session["profile_photo"] = id_info.get("picture")  # Set the profile photo if available
    session["email"] = id_info.get("email")  # Set the email if available
    session["phone_number"] = id_info.get("phone_number")  # Set the phone number if available
    ...
    # User not registered
    return render_template("signup.html", name=session["name"])
...
def home():
    ...
    # Get the login timestamp and format it as desired
    login_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    return f"Welcome to the home page, {username}!<br/>Login timestamp: {login_timestamp}<br/><a href='/logout'><button>Logout</button></a>"
